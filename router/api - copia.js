...
router.post("/ordenes_nueva", async(req,res)=>{
   ...
   SELECT '${empnit}' AS EMP_NIT
      ...
      ,'${coddocent}' AS CODDOC
      ,'${correlativoentada}' AS [DOC_NUMERO]
      ,[CODCAJA]
      ,CONVERT(VARCHAR(10),GETdate(),101) AS [DOC_FECHA]
      ,'${coddocsal}' AS DOC_NUMREF
      ,'${correlativosalida}' AS DOC_NOMREF
      ,[BODEGAENTRADA]
      ,[BODEGASALIDA]
      ,'GIO_PROD' AS USUARIO
      ,'O' AS DOC_ESTATUS
      ...
      ,[DOC_TOTALOTROS]
      ,[DOC_RECIBO]
      ,'${codprod} - ${desprod}' AS DOC_MATSOLI
      ,[DOC_REFERENCIA]
      ,[DOC_LUGAR]
      ,[DOC_ANOMBREDE]
      ,[DOC_IVAEXO]
      ...
      ,[FIRMAELECTRONICA_A]
      ,[IDELECTRONICA_A]
  FROM [dbo].[Documentos] WHERE CODDOC='PRO00' AND EMP_NIT='6904684-0' AND DOC_NUMERO='     11396';
	`;
...
function qry_insert_docproductos_entrada(empnit,coddocent,correlativoent,codprod,cantidad){
  ...
  1 AS EQUIVALE, 
                         ...
                         0 AS NOAFECTAINV, '' AS PRODFVENCE, 0 AS DOCPESO, 0 AS COSTOINV, 0 AS FLETE, 0 AS TOTALPRECIOFIN, 0 AS PRECIOFIN, 0 AS TOTALCOSTOINV, 0 AS CANTIDADBONI, ' ' AS CODOPR, ' ' AS NUMOPR, 0 AS ITEMOPR, 
